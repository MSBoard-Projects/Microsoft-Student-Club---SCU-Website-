...
function GoldenGroups({ people, month = 'all' }: { people: GoldenRecipient[]; month?: string }) {
  ...
  return <>{(Object.keys(categories) as GoldenCategory[]).map(category => {
    ...
    return group.length ? <section key={category} className="club-golden-group" aria-label={categories[category]}><h2>{categories[category]} <span>{group.length}</span></h2><div className="club-golden-grid">{group.map(person => {
      const member = members.find(item => item.id === person.memberId);
      ...
      return <article key={person.id} className="club-golden-card">
        <div className="club-golden-photo">{member?.imageUrl ? <OptimizedImage src={member.imageUrl} alt={person.name} fit="contain" aspectRatio="1" framed={false} sizes="(max-width: 760px) 100vw, 360px" /> : <span className="club-member-initials" aria-hidden="true">{person.name.split(/\s+/).slice(0, 2).map(part => part[0]).join('')}</span>}<span className="club-golden-badge"><Icon glyph={FiAward} />{person.awards.length} {person.awards.length === 1 ? 'recognition' : 'recognitions'}</span></div>
        <div className="club-golden-copy"><h3>{member ? <Link to={`/members/${encodeURIComponent(member.id)}`}>{person.name}</Link> : person.name}</h3><p>{award.role}</p><div className="club-golden-months">{person.awards.map(item => <span key={item.month}>{monthLabel(item.month)}</span>)}</div></div>
      </article>;
    })}</div></section> : null;
  })}</>;
}
...
export default function GoldenMembers() {
  ...
  const people = (recurring ? recurringRecipients : goldenRecipients).filter(person => {
    ...
    return award && (category === 'all' || award.category === category) && `${person.name} ${award.role}`.toLowerCase().includes(query);
  });
