...
export function Counter({ value }: { value: number | null }) {
  ...
  return (
    <span ref={element} className="club-counter" aria-label={target === null ? 'Not published' : target.toLocaleString('en-US')}>
      <span aria-hidden="true">{target === null ? '--' : (reducedMotion ? target : displayValue).toLocaleString('en-US', target >= 10000 ? { notation: 'compact', maximumFractionDigits: 1 } : {})}</span>
    </span>
  );
}
...
export default function StatisticsBanner({ statistics }: { statistics: ClubStatistics }) {
  const reducedMotion = useReducedMotion();
  const metrics = [
    { key: 'registeredAttendees', label: 'Total Registered Attendees', icon: FiUsers, value: statistics.registeredAttendees },
    { key: 'eventLocations', label: 'Event Locations', icon: FiMapPin, value: statistics.eventLocations },
    { key: 'beneficiaries', label: 'Total Beneficiaries', icon: FiHeart, value: statistics.beneficiaries },
    { key: 'eventsConducted', label: 'Total Events Conducted', icon: FiCalendar, value: statistics.eventsConducted },
  ].map(metric => ({ ...metric, value: metric.value !== null && Number.isSafeInteger(Math.floor(metric.value)) && metric.value >= 0 ? metric.value : null }));
        ...
        <div className="club-stats-grid">
          {metrics.map((metric, index) => (
            <motion.div key={metric.key} className={`club-stat club-stat-${index}`} initial={reducedMotion ? false : { opacity: 0, y: 18 }}
              ...
              {metric.key === 'eventsConducted' && metric.value !== null && <span className="club-stat-status">Completed events in the published catalogue</span>}
            </motion.div>
          ))}
        </div>
      </div>
